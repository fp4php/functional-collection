...
final class NonEmptySetCollectorTest extends TestCase
{
    ...
    {
        $this->assertEquals([1, 2, 3], NonEmptyHashSet::collect([1, 2, 3])->getUnsafe()->toNonEmptyList());
        $this->assertTrue(Option::try(fn() => NonEmptyHashSet::collectUnsafe([]))->isNone());
    }

    public function testCollectUnsafe(): void
    {
        $this->assertEquals([1, 2, 3], NonEmptyHashSet::collectUnsafe([1, 2, 3])->toNonEmptyList());
        $this->assertTrue(Option::try(fn() => NonEmptyHashSet::collectUnsafe([]))->isNone());
    }
    ...
    public function testCollectNonEmpty(): void
    {
        $this->assertEquals(
            [1, 2, 3],
            NonEmptyHashSet::collectNonEmpty([1, 2, 3])->toNonEmptyList(),
        );
    }
